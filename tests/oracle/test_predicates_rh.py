...
def test_repeat_too_fast_flagged() -> None:
    ...
    diagnostics = check_right_hand(t, MEDIAN_HAND, tempo_bpm=120.0)
    repeat = [x for x in diagnostics if x.violation_type == "RIGHT_HAND"]
    assert repeat and repeat[0].overage > 0
