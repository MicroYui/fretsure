...
- **下一步已冻结**：Task 8 attempt-002 已 COMPLETE 8/8；不得用失败 attempts 的缺失 usage 冒充零，也不得拿 pilot outcome 调参。Task 9 attempt-001 已 terminal `INCOMPLETE`，不得 resume 或覆盖；保留它的 pre-call、旧 gate、WAL 和 abort receipt。修正 official-output-usage envelope、gate 与 runner 后，在 clean pushed SHA 上生成 fresh attempt-002 pre-call，再运行正式采集和两次独立 full replay。运行时不调用 Git/子进程。完整 Plan 6B 仍后置；新的前端/音频审美、真人演奏或 calibration gate 需先请求用户确认。
...
16. **Benchmark v2（2026-07-17）**：历史 Plan 4 runner 只保留为骨架；新计划先冻结逐 item/candidate
    ...
    已 COMPLETE；Task 9 runtime input guard、externally declared ceiling gate 和 raw-only collection/replay 边界已实现。formal attempt-001 已 terminal `INCOMPLETE`；修正官方 output-usage 契约后从 fresh attempt-002 继续。
...
- Benchmark v2 Task 7 已闭环，Task 8 attempt-002 也已 COMPLETE 8/8；不要检查私密 prompt/response，也不要把失败 attempts 的缺失 usage 写成零。Task 9 已获统一计费授权；历史 `$538,865.486400` gate、attempt-001 pre-call/WAL/abort receipt 均保留且不复用。按官方 128k output-usage 契约修正后，新机械最大值是 `$1,167,905.640000`；继续时从 clean pushed runner SHA 生成 fresh attempt-002 pre-call，再正式采集和双 full replay。新的前端/音频审美、真人听感或 calibration gate 仍需用户确认。
