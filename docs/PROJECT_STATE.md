...
**当前数字，`oracle@0.8.0` / `median@0.3`，全部于 2026-07-30 重测：**

```
曲目门（固定变调夹） 149 / 292 = 51.0%   GREEN 114 · AMBER 35 · INFEASIBLE 143
曲目门（capo ladder）173 / 292 = 59.2%   GREEN 129 · AMBER 44 · INFEASIBLE 119
冻结基线            30 / 56（固定）· 32 / 56（capo）
帧级假阴性           3.3%（全语料 241 帧判定，7 帧被拒，全部 FRET_SPAN）
判别力               96.7 个百分点
负样本守卫           RED 1439 · AMBER 197 · GREEN 82，invariant_held
```

**报曲目门必须标注模式。** 按历史一直在用的那一列（capo ladder）读，本周是
**146 → 173 / 292，50.0% → 59.2%，GREEN 89 → 129**——这才是可比的数字，也比固定变调夹
那一列（110 → 149）看起来的要好。capo ladder 在当前 oracle 下本身值 **+24 首**，
而且它稀释的是**所有** infeasible 桶（frame-config 44 → 38，beam 93 → 75），不是某一个。
