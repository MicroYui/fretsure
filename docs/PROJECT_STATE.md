...
> 目的：任何新会话读完本文件 + 设计 spec，即可无损接上。最后更新：2026-07-23。

## 0. 当前恢复真源

当前工作分支是 `codex/benchmark-v2-task10`。Task 9 已完整闭门并合并/推送到 `main`
`6683183e0d09eff3208be70e425db4ae6714ee3d`；正式进程、pilot 和小时监控均已结束，不得重启
attempt-004。Attempts 001–003 仍是不可恢复、不可覆盖的 `INCOMPLETE` 历史证据，累计
known/tight cost `$2.130022 / $804.234022`，不进入最终分析。

Task 10 的结果发布、验收文档、fresh release gates 与独立复核均已完成，正在做 Git closure。当前正式决策为：repair
...
integrity/authorized replay，不主张 public rescore 或 remote durability。Task 10 在
commit/push、fast-forward `main` 以及
local/tracking/remote SHA equality 全部闭合前不得开启后续计划。真人 empirical 与公开完整 replay
包继续保持 OPEN。
...
deterministic analysis 与双 replay 验收已闭合；Task 10 只剩 Git closure。
...
- **下一步已冻结**：完成 Task 10 closure commit/push 与 `main` SHA equality；然后在真人/许可等 OPEN gate 处暂停等待用户指示。不得重启 Task 9 collection。完整 Plan 6B 仍后置；新的前端/音频审美需先请求用户确认。
