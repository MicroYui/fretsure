...
| W4 | 滚奏/扫弦手势（`oracle@0.4.0`、`tab-input@0.3.0`、`fingering-solver@0.7.0`） | 26/58 |

剩余失败（32 首）：beam 内失败 23、无可行帧配置 8、单 onset 音数超过弦数 1。
待做：W6 用曲目 + 负例双侧校准出 `repertoire@0.1` profile 并设为默认、W7 逐桶汇报并免费重跑
确定性 benchmark baseline。完整计划见会话计划文件；实测与决策记录见
