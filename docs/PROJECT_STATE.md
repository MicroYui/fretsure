...
**上一版记的 146 / 292（GREEN 89）复现不出来。** 同一 commit、干净工作树，跑两次
都是 107 / 292（GREEN 67）。唯一可能解释四十首差距的开关 `--choose-capo` 在冻结
56 首上只多救回 1 首。来源无法重建，所以是**改正**而不是解释。
教训很便宜：**没人能重新签发的数字不是基线**——曲目门一臂要 27 分钟，于是它被跨会话
凭记忆引用而不是重测。
