...
> 目的：任何新会话读完本文件 + 设计 spec，即可无损接上。最后更新：2026-07-30。

## 0. 当前恢复真源（2026-08-01）

**当前数字，`oracle@0.8.0` / `median@0.3`，全部于 2026-08-01 在修复后的语料上重测：**

```
曲目门（固定变调夹） 154 / 292 = 52.7%   GREEN 117 · AMBER 37 · INFEASIBLE 138
曲目门（capo ladder）178 / 292 = 61.0%   GREEN 132 · AMBER 46 · INFEASIBLE 114
曲目门（完整 escalation）182 / 292 = 62.3%  GREEN 135 · AMBER 47 · INFEASIBLE 110（上限）
冻结基线            30 / 56（固定）· 32 / 56（capo）
帧级假阴性           4.1%（245 帧判定，10 帧被拒，全部 FRET_SPAN）
判别力               95.9 个百分点
负样本守卫           RED 1439 · AMBER 197 · GREEN 82，invariant_held
语料音域审计         19 → 1（唯一剩下的是具名隔离的 aguado-op03n05）
```

**帧级从 3.3% 变成 4.1%、判别力从 96.7 降到 95.9，是因为语料被修正了**——多了 3 帧真实
可判定的编者帧，而它们下面的曲子不再高八度。**指标变差是因为它现在测的是正确的数据。**

**报曲目门必须标注模式。** 按历史一直在用的那一列（capo ladder）读，这两周是
**146 → 178 / 292，50.0% → 61.0%，GREEN 89 → 132**——这才是可比的数字，也比固定变调夹
那一列（110 → 154）看起来的要好。capo ladder 在当前 oracle 与修复后的语料上本身值
**+24 首**，而且它稀释的是**所有** infeasible 桶（frame-config 46 → 40，beam 91 → 73），
不是某一个。

**曲目门与帧级现在会朝相反方向动，必须一起报。** 斜角豁免让帧级从 5.3% 降到 3.3%，
曲目门却从 152 掉到 149——`no feasible frame config` 44 不变，全部差额落在
`no non-red extension within beam`（90 → 93）。**没有东西变得弹不了**：每帧存活的
配置变多、宽度 16 的 beam 更挤，原本能走到底的前缀被更便宜的死路挤掉。这是
`score.py` 已记录的 beam 非单调性，也意味着**曲目门当前不是对物理模型的干净读数**。

**曲目门有两个模式，共用一个名字。**（2026-07-31 更正）历史上记的 146 / 292 是
**开 `--choose-capo`** 的数字；上面这些以及 07-30 全部测量都是**固定变调夹**的。
我 07-30 曾断言 146「复现不出来、故改正」——**那个断言是错的，现予撤回**。

判定不靠推理：两次测量在**机制不起作用的地方逐个 id 完全一致**，在起作用的地方才分叉。
`2026-07-28` 冻结工件与我在 `d753751` 上的重跑在冻结 56 首上都是 26/56、接受的是同样
26 首；全语料却差 33 首。而冻结 56 首正是 capo 唯一没用的地方（31 首被拒只救回 1 首），
扩展语料才是它生效的地方（一个文件上 14 首被拒救回 6 首，43%）。

我当初排除 capo，用的正是那 56 首子集——**去否定一个关于 292 首的主张**。详见
`docs/experiments/2026-07-31-gate-had-two-modes.md`。

**语料是 292 首，不是 389。** 389 行里 86 首是同一段音乐算了两遍（Mutopia 源惯用
`\score{\layout}` + `\score{\midi}` 两块，乐章切分器当成了两个乐章），另有 11 首
从未被正确解析（同一 onset 音数超过弦数）。冻结基线因此是 **56 不是 58**。

### 2026-07-30：手位窗口就是跨度规则，而且用错了常数

`check_shift_speed` 给每个按弦音一个手心区间 `[press_x ± reach_mm]` 再取交集——
非空当且仅当**最远的一对沿颈相距 ≤ 2 × reach_mm**。这与 `check_fret_span` 的 `d_max`
是同一条物理主张，只是**抹掉了手指编号**。两个常数管一件事，小的那个生效：

```
                最宽 d_max     2 × reach     实际生效
悲观（定 GREEN）   117.0 mm      90.0 mm       90.0
乐观（定 RED）     143.0 mm     110.0 mm      110.0
```

(1,4) 指对的实际许可是 110 mm 而不是 143。**07-29 那次「常规伸展技术」的决定进了
文档但没进判定器**——它仍然拒绝手指 1、4 按住 3 品和 7 品，那正是那个伸展形状本身，
只是往上挪了一个把位。这个缺口是 07-29 我自己开的：抬了 span 却按住 reach，理由是
「reach 是手待的地方、span 是能撑多远」——那个理由是错的，窗口不是休息位规则，
就是同一条伸展规则抹掉手指。

修法：`reach_mm = hand_span_mm / 2`，并把不变量恢复成相等，用「窗口必须是精确规则的
**松弛**」来论证，且用**穷举单帧**的性质测试钉住（`test_the_hand_window_never_
refuses_a_frame_the_exact_rules_admit`）。

**这次没有靠肉眼判形状**。218 张负样本 tab 移动，595 条被放弃的拒绝逐条对照了
拥有同一限制的那条规则：495 条点名 ≥2 个按弦音，**495 条全部落在它自己那对手指的
许可之内**；100 条只点名 1 个音（来自跨时间携带的区间），**100 条的距离全部在
`v_shift` 允许的行程之内**，好几条差一个数量级（需要 35.3 mm，可用 366.7 mm）。
**没有一条是「换把太快」，全部是「区间窄到装不下一只手」。**

### 2026-07-30：beam 宽度重测，结论比原来更强

`no non-red extension within beam` 是 93/292，最大的一桶。宽度这条路本周被判过死刑
（1/16），但那是在**有 30 mm 缺陷的 profile** 与**拒绝手腕几何的单调规则**下测的，两者都让
beam 远没有现在拥挤，所以重跑而不是继承。

```
            accepted    GREEN   beam 桶   frame-config 桶   segment 预算   耗时
beam 16     149/292      114      93           44               0        35 min
beam 32     143/292      111      94           42               5        65 min
```

**两点结论。** 一、**加倍宽度并没有缩小它该缩小的那一桶**（93 → 94）——杀死搜索的不是
「只留了 16 个」，而是留了**哪** 16 个。宽度与保留策略是两条独立的杠杆，前者现在测平了。
二、代价落在别处：beam 32 有 5 首栽在 score-level segment 预算上（每 config 工作量翻倍
→ 聚合工作量门耗尽分段），这是混淆项；但**即便把这 5 首全算成功也只有 148 < 149**，
beam 32 在自己的条件下就已经输了。

**这重新打开了保留策略那条路**：我曾用「更宽的 beam 涵盖了 ranker 能保留的东西」关掉它，
那个推理是错的——beam 按代价与多样性保留，ranker 会按**预测可完成性**重排。现在有直接
证据说明两者不同，因为宽度杠杆已经测平。是否值得做是另一个问题（监督信号「这个前缀是否
走到了被认证的结尾」正是曲目门要跑 35 分钟才能算出来的东西）。

`MAX_SCORE_SOLVER_SEGMENTS` 与 beam 都不动。

### 2026-07-30：两个 v1 占位参数不需要出处，因为它们不改变任何答案

```
v_shift_mm_per_s   200 → 2500 mm/s 判决多重集完全不变（出厂 500）
r_max_hz             4 →   40 Hz   判决多重集完全不变（出厂 8）
```

`SHIFT_SPEED` 之所以**看起来**吃重，是因为一个诊断名覆盖了两个常数，而另一个
（`reach_mm`）设错了。修完之后剩下的 5,734 条 shift 诊断里 354 条确实超过 `v_shift`，
所以它在谓词里不是死代码，但在**结果**上不生效。

### 本轮修掉的两个真缺陷（2026-07-29）

**`d_max` 比琴颈还窄**（`oracle@0.6.0`）。同一品横跨六弦是 52.5 mm，而 (2,3)/(3,4)
指对只给 50 mm——**G 大和弦在 `median@0.1` 下是 AMBER、在 `small@0.1` 下是 RED**。
下界改为琴颈宽度，依据是乐器而非语料。

**跨度按直线而非沿颈计算**（`oracle@0.7.0`）。`dist = abs(pa[0]-pb[0])`，一行。
零新参数、零拟合常数。两个 split、两端同时变好：真人指法被拒得更少，**同时**被拉伸
的形状抓得更严——跨弦分量原本在稀释真正该拒的形状的限制。

两者都**不是**调参：一个来自乐器几何，一个来自度量形式。

### 本轮建立的仪器（这是最该保留的东西）

`scripts/measure_oracle_discrimination.py`——两侧都来自出版谱本身，**不需要真人**：

- **正例** = 编者印在谱上的指法（249 帧），问「与这些指号相容的任何 (弦,品) 读法能否通过」
- **负例** = 同一段音乐把一个音沿颈推开，1 / 2 / 3 / 6 / 12 品

判据是**曲线形状**，不是单点阈值：零位移处该接近全接受，远场该接近全拒绝。
它几分钟跑完（整曲门控要一小时），并且能定位到具体的帧和具体的规则——**今天两个
真缺陷都是它找出来的**。

**读法限制**：中段（1–3 品）不是干净的负例，推开一两个品的形状往往仍然能弹，
所以该看两端读。

### 为什么整曲 50% 而帧级 85% 正确

**误差连乘。** 一首曲子约 150 帧，整曲通过要求每帧都过：

```
0.49 = x^150   →   每帧需要 99.5%
49% → 80%      →   每帧误差要降到三分之一
```

推论：**追整曲覆盖率的大改动是徒劳的**，不存在能一次搬动几十首的旋钮；有价值的是
消除**系统性的每帧缺陷**，因为它们的效果是连乘的。这也解释了为什么本轮九个搜索侧
方向全部关闭，而两个帧级缺陷的修复才是真正的产出。

### 146 次拒绝的完整归因

```
101   beam 死亡（69%）
 28   帧几何，最接近的合法放置还差 11.0 mm（最小）/ 25.8 mm（中位）
 14   真无解（音高无法占据不同弦 10）/ 需要第五指 2 / 历史问题 2
  3   音高不可达（变格定弦未记录，修了定弦也解不出来）
```

**42 首「无可行帧配置」全部判定完毕**（不是 25 首）。两个替代解释被直接测掉：
`frame_configs` 在 400 倍宽度（20000）下对 17 个密集帧仍返回**零配置**；42 首
**没有一个**需要超过 4 个不同品位。裕度太宽，任何可辩护的常数都够不到。
...
| 保留质量（随机化） | 0 / 3 公平试验 |
| 生成宽度 | 8 倍，1 / 12 |
| 逐对 DP 找路径 | 3 / 12 找到，**3 / 3 在完整检查器下失败**（SHIFT_SPEED 非马尔可夫） |
| 持续音放宽 | 4 / 15 可解，**全部低于 0.90 保真度下限** |
| 位移速度 | +40%，train 201 首得 0 失 0 |
| 加大 hand_span | 编者证据反推需求 60–259 mm（是噪声），且 RED 退化 |
| 降低保持率下限 | 编者指号蕴含的保持率最低 0.955，**不支持** |
| 手掌平面模型 | 20 个工作点 × 2 split，**无一占优** |
| 斜角豁免（4 个界） | 四种互不相关的界，**交换汇率相同** |
| 相邻指对因子 | train 有效（14.7→13.2），**test 完全无效**——不发布 |

**其中五个是在动手实现之前关掉的。** 单是 completability ranker 就是数周工作。

### 唯一还开着的帧级缺陷

**`FINGER_MONOTONIC` 拒绝手腕的自然斜角**——它比较指号与品位顺序时不看弦号，
而手腕横跨琴颈时，伸向高音弦的手指自然更靠近琴枕。它出现在 36 个剩余被拒帧的
**35 个**里，单独拒绝 10 个，**基本就是那 15% 的全部**。

四个界都试过，交换汇率相同：**单调规则和跨度规则不冗余**，它约束的「手指不能互相
穿过」不是一个距离，所以任何基于距离的界都复现不出它。

**解锁条件不是更好的规则组织，是新的证据类型**：同一批曲目的**其它编者版本**
（许可 + 解析工程），或真人测量（给参数一个出处）。

### 转换器修复（`python-ly-string-numbers@0.1.0`）

python-ly **认得**弦号（`ly.lex.lilypond.StringNumber`）却从不导出——619 个弦号
在 31 个源文件里写着，语料收到 0 个。补上后 **494 个标注带弦号**。

重建前先立安全性质：**每一首的音符流必须逐字节相同**，292 首全部通过，编者帧集合
不变，全语料判别曲线逐项相同。

**但它没有解决斜角**：同时带手指和弦号、且同一 onset 上有两个这种音的帧只有 **5 个**。
编者标弦号是为了提示换把位，不会标满和弦。

### 一条过程教训（对下一个会话有用）

本轮我在测量上犯了**五次同类错误**，全部是「报告的不是被测的东西」：

1. 位移负例**重新枚举**指法（三次），使三分之一米的伸展看起来可弹——正确做法是取
   位移 0 时通过的那个实现再推开它
2. 用会**应用被测规则**的过滤器取实现，于是倒序按构造不可能出现
3. 把和弦拼写成**六次拇指拨弦**（三次），右手规则全拒，报出「判定器拒绝 100% 真人指法」
4. 跨度归因取**最贴合**的指对而非**绑定**的那一对，报出「FRET_SPAN 拒绝了一个舒适地
   在限内的几何」
5. 独立枚举手指，导致 17 个帧「枚举过大」——而单调规则**决定**了指号，多乘了 4ⁿ

每一次都是先看起来像结论、再被自己的数据推翻。**任何测量在报出去之前，先问一遍：
如果被测对象是完美的，这个脚本会输出什么？**
