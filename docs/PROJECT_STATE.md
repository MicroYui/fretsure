...
## 0. 现状一句话
设计已锁定；**Plan 1–5、Pre-Plan 6 MusicXML-first、Oracle 0.2、安全 `.mxl`、Plan 6A、producer-driven MusicXML/IR、strict MIDI input 与 benchmark v2 Task 1–8 均已闭门；Task 9 formal runner 已实现，正式采集尚未启动**。MIDI 的 local/tracking/remote receipt 一致指向 `46ff8ac070e97422b4aecf5c0f2a22b588a5fda4`。benchmark v2 已冻结 500 个 procedural families + 3 个许可 public controls、十槽共享候选池、raw/pure baselines、ITT/统计、WAL、版本化 artifact/report、机器预注册、attempt-local pre-call 与离线 replay；Task 8 另冻结了 2×2 operational pilot、精确 pricing/budget 算术、clean resume 与显式 spend confirmation，见 [`BENCHMARK_V2_TASK8_READINESS.md`](BENCHMARK_V2_TASK8_READINESS.md)。当前 package=`0.6.0`、router=`score-input@0.1.0`、importers=`musicxml@0.3.0` / `midi@0.1.0`、faithfulness=`fidelity@0.3.0`、trace=`agent-trace@0.2.0`、service=`fretsure-service@0.2.0`、API=`fretsure-api@0.2.0`、MCP=`fretsure-mcp@0.2.0`、Web=`fretsure-web@0.2.0`；playability=`oracle@0.2.0`、公共输入=`tab-input@0.2.0`、container=`mxl-container@0.1.0`、profile=`median@0.1` 保持不变，runtime 精确锁定 `music21==10.5.0`。Task 8 attempt-002 已 COMPLETE 8/8；两次 pilot 合计已知 / tight upper 为 `$0.513140` / `$1.280116`，失败 attempts 的缺失 usage 仍为 unavailable。formal envelope SHA-256=`5bcd24585db7a062955b2dc3de543e8ecc7e875c4647b6d767e348ee1cb15b5d`，冻结每个 input/cache bucket `272,000`、output `16,384`。用户已统一授权本项目模型计费；Task 9 私有 external-ceiling gate SHA-256=`931b5ae14d587d89511aa3b5c45c7458e96c377df54093ad6244a14948527bd9` 精确声明 `$538,865.486400` 上限，但 gate 本身不授予调用权限。formal runtime 已在 observation/WAL/retry/network 前执行 UTF-8 bytes + 256 guard，要求 live CLI 重复精确金额，成功调用返回精确模型证据，live 只发布五个 raw canonical 工件并由两个独立 full replay 生成报告。运行时不调用 Git/子进程。本阶段没有前端改动。
...
- **下一步已冻结**：Task 8 attempt-002 已 COMPLETE 8/8；不得用失败 attempts 的缺失 usage 冒充零，也不得拿 pilot outcome 调参。Task 9 guard、gate binding、精确 spend confirmation、provider evidence abort 与 raw-only finalization 已实现；下一步是在 clean pushed runner SHA 上生成 attempt-001 pre-call，运行正式采集，再做两次独立 full replay。运行时不调用 Git/子进程。完整 Plan 6B 仍后置；新的前端/音频审美、真人演奏或 calibration gate 需先请求用户确认。
...
16. **Benchmark v2（2026-07-17）**：历史 Plan 4 runner 只保留为骨架；新计划先冻结逐 item/candidate
    ...
    已 COMPLETE；Task 9 runtime input guard、externally declared ceiling gate 和 raw-only collection/replay 边界已实现，formal collection 尚未启动。
...
- Benchmark v2 Task 7 已闭环，Task 8 attempt-002 也已 COMPLETE 8/8；不要检查私密 prompt/response，也不要把失败 attempts 的缺失 usage 写成零。Task 9 已获统一计费授权并生成 exact `$538,865.486400` 私有 gate；继续时从 clean pushed runner SHA 生成 attempt-001 pre-call，再正式采集和双 full replay。新的前端/音频审美、真人听感或 calibration gate 仍需用户确认。
