...
| W3 | 整曲持续音释放阶梯（`score-solver@0.6.0`、`sustain-model@0.2.0`） | **26/58** |

剩余失败（32 首）：beam 内失败 21、无可行帧配置 8、同时攻击超过右手指数 3。
待做：W4 滚奏/扫弦（含 Tab wire schema 升级、`oracle@0.4.0`）、
W6 用曲目 + 负例双侧校准出 `repertoire@0.1` profile 并设为默认、W7 逐桶汇报并免费重跑
