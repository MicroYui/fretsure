...
重跑 units 均已越过。当前按用户要求干净暂停在 3,705 READY、4,208 rows / 13,840 completed
calls（36.83%），extra attempts=439，无 abort；PID/screen 均不存在，automation=`PAUSED`。
下一步只能在用户明确指示后同目录 resume；后续仍约每 5% 与 terminal 更新证据，完成后才做
双离线 replay。正式与 pilot 在建客户端前机械要求数值 loopback，拒绝 `localhost`。
