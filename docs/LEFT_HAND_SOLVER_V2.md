...
物理 Oracle 的相邻/隔指触及占位模型也从线性 `gap / 3` 改为单调的 `0 / 0.50 / 0.90 / 1.00` 手跨度。旧模型会把标准开放位 C、F 和弦判成 AMBER，同时把高把位替代按法判成 GREEN；`oracle@0.4.0` 已用标准和弦回归修正这一反例。GREEN 仍只是版本化模型证据，不是对所有真人的保证。
