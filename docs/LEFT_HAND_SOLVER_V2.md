...
物理 Oracle 的相邻/隔指触及占位模型先从线性 `gap / 3` 改为按**指号差**的 `0 / 0.50 / 0.90 / 1.00`
手跨度——旧模型会把标准开放位 C、F 和弦判成 AMBER，同时把高把位替代按法判成 GREEN，`oracle@0.3.0`
用标准和弦回归修正了这个反例。**`oracle@0.5.0` 又把它从「指号差」换成「指对」**：指号差让
(1,2) 和 (3,4) 拿到相同额度，而 389 首出版谱上仅因 span 被拒的帧里 **13/16 绑在相邻指对**。
只有 `(1,2)` 从 0.50 动到 0.55，实测 +2 首曲目、**零倒退**、负例上 **RED→GREEN 为 0**（新增的 4 份
认证全部来自 AMBER 带）。GREEN 仍只是版本化模型证据，不是对所有真人的保证。
