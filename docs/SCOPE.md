...
> model-relative certification against `oracle@0.5.0`, `tab-input@0.3.0`, and a
...
> `oracle@0.5.0` found the exhibited fingering feasible under the parameters it
...
`oracle@0.5.0` is defined for **valid, ordinary six-string `Tab` values**. The public
...
`Tab` still passes a complete final `oracle@0.5.0` check, so incompleteness cannot leak
a RED result.
...
`oracle@0.5.0` and `tab-input@0.3.0`, while the bundled preset remains
...
snapshot; they are not results under the current `oracle@0.5.0` / `fidelity@0.3.0`
checker pair.
