...
[`docs/PLAN6A_ACCEPTANCE.md`](docs/PLAN6A_ACCEPTANCE.md)，当前 MIDI final gates 见
...
**Plan 1–5、Oracle 0.2 软件信任门、安全 `.mxl`、Plan 6A、producer-driven MusicXML/IR 已闭门；
strict MIDI input 的实现与 producer corpus 已落地，最终仓库/分发/review/commit/push/SHA gates 仍在进行。**
当前 package=`0.5.0`、router=`score-input@0.1.0`、importers=`musicxml@0.3.0` / `midi@0.1.0`、
...
`music21==10.5.0` 精确锁定。MIDI 的两正两负 exact producer rows、资源门、诚实限制与尚未填写的
最终 gates 见 [`docs/MIDI_ACCEPTANCE.md`](docs/MIDI_ACCEPTANCE.md)。完整 Plan 6 的音频、AlphaTab、
真实琴颈动画、导出、live A/B/榜单与真人 money moment 仍 open；只有 MIDI 阶段提交、推送并核对
local/tracking/remote SHA 后，benchmark v2 才成为下一阶段。
