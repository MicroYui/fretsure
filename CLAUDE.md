...
**当前阶段（2026-07-18）**：**Plan 1–5、MusicXML-first、Oracle 0.2、安全 `.mxl`、Plan 6A、producer-driven MusicXML/IR、strict MIDI input 与 benchmark v2 Task 1–7 runner-ready 软件门均已闭门；尚未发起真模型收集**。当前运行 package=`0.6.0`、router=`score-input@0.1.0`、importers=`musicxml@0.3.0` / `midi@0.1.0`、faithfulness=`fidelity@0.3.0`、trace=`agent-trace@0.2.0`、service=`fretsure-service@0.2.0`、API=`fretsure-api@0.2.0`、MCP=`fretsure-mcp@0.2.0`、Web=`fretsure-web@0.2.0`；checker/input/container/profile 保持 `oracle@0.2.0`、`tab-input@0.2.0`、`mxl-container@0.1.0`、`median@0.1`，runtime 精确锁定 `music21==10.5.0`。Task 7 已冻结 notegraph/corpus/config-manifest/row/sanitized-observations/receipt/report、机器预注册、attempt-local pre-call 与离线 replay 合同；Task 8 的真实 pilot/价格授权仍 open。默认真代理模型是 canonical `gpt-5.6-sol`，只有显式有效的 loopback proxy 与 pre-call 配置才可联网。本阶段没有前端设计改动；若要 dashboard、图表页或 live leaderboard，先与用户确认“古典制琴工坊 × 验证仪器”视觉。完整 Plan 6 的音频/AlphaTab/琴颈动画/导出/live demo 与真人 money moment 仍 open。
