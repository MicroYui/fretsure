...
## 当前数字（2026-08-01，唯一权威处；下面的历史段落只解释来由，不要从中取值）

```
oracle@0.8.0 / median@0.3
曲目门 固定变调夹   154 / 292 = 52.7%   GREEN 117 · AMBER 37 · INFEASIBLE 138 · 冻结 30/56
曲目门 capo ladder  178 / 292 = 61.0%   GREEN 132 · AMBER 46 · INFEASIBLE 114 · 冻结 32/56
曲目门 完整 escalation 182 / 292 = 62.3%  GREEN 135 · AMBER 47 · INFEASIBLE 110 · 冻结 33/56（上限）
...
**报曲目门必须标注模式**——两个模式是两个指标，共用一个名字曾导致一次错误的"更正"。
按历史一直在用的 capo 那一列读，这两周是 **146 → 178（50.0% → 61.0%），GREEN 89 → 132**。
...
**曲目门与帧级会朝相反方向动，必须一起报**（07-30 实例）。斜角豁免把帧级从 5.3% 压到 3.3%，曲目门
...
向外推，并加了回归测试。（当时帧级 9.8%、判别力 90.2 点，均为语料修复前的读数。）
...
音域审计 **19 → 1**。两个模式各 +5 首 +3 GREEN，"音域外" 6 → 1。
...
**beam 宽度已彻底关闭**（08-01，完整 escalation 首次跑完）：capo 阶梯 +24 首，
**beam 阶梯（32 然后 64）只 +4 首**，即它瞄准的 73 个 beam 死亡里只救回 5.5%，
代价是再多 55 分钟。剩下 69 个**不是宽度受限**。所以这个语料里唯一还开着的杠杆是
**beam 保留哪些状态，不是保留几个**——`score.py` 自己写着 *"cost does not predict
completability"*，现在支撑它的数字是「把搜索翻两番只换来 4 首」。

**beam 宽度在新条件下重测**（07-30，较弱的早期证据）：beam 32 是 **143/292**，比 16 的 149 更差，
