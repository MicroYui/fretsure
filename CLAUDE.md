...
**固定变调夹 149/292 = 51.0%**（GREEN 114、冻结 30/56），**capo ladder 173/292 = 59.2%**
（GREEN 129、冻结 32/56）。**帧级假阴性 3.3%**、判别力 96.7 点。
...
音域审计 **19 → 1**。曲目门 **149 → 154/292 = 52.7%**（GREEN 114 → 117），"音域外"6 → 1。
...
**弦号编号方向是 1 = 最低弦**（校准出来的，一致率 98.9% vs 反向 80.5%），
且 `python_ly_string_numbers.py` 有**和弦归属 bug**：`<cis-4 e,-3\3>` 里的 `\3` 属于 `e,`，
语料挂到了 `cis` 上（476 处标注里 5 处）。帧级配置那 44 帧重新归因：19 几何（裕度 3.4 / 21.3 / 67.2 mm）、
